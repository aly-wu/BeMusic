...
public class BeMusicDatabase implements UserDatabase {
    ...
    private HashMap<BeMusicUser, ArrayList<BeMusicUser>> adjList; // adjacency list, keys with users, values with
                                                                  // friends
    // TODO: perhaps we do a cusotm hashing thing that hashes BeMusicUsers based on
    // their username, not just if they are distinct objects which all users are
    ...
    public void addVertex(BeMusicUser v) {
        if (!validateVertex(v)) {
            ...
            System.out.println("username " + v + " already taken.");
        }
    }
     ...
     * 
     * An edge can only be made if their list of friends exists in the database.
     * Simply
    ...
    /**
     * Adds the undirected edge v-w to this graph so long as it does not already
     * exist.
    ...
    public void addEdge(BeMusicUser v, BeMusicUser w) {
        if (validateVertex(v) && validateVertex(w) && !adjList.get(v).contains(w)) {
            ...
            adjList.get(v).add(w);
            adjList.get(w).add(v);
            // NOTE: do not need to re-put into adjList when adding to exisitng list of
            // friends as ArrayLists are mutable objects
        }
    }
    ...
    /**
     * Remove a vertex v, removing all edges to v as well. Does nothing if v is not
     * a real user.
